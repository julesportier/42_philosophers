...
/*   Updated: 2025/06/26 10:17:59 by juportie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	join_threads(const pthread_t *threads, int philos_nbr)
{
	int	i;
	...
	int philos_nbr)
{
	if (create_threads(threads, philos, philos_nbr)
		== ERROR)
		return (ERROR);
	if (join_threads((const pthread_t *)threads, philos_nbr)
		== ERROR)
		return (ERROR);
	return (0);
}
